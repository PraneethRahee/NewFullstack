...
import UnsplashImagePicker from "@/components/image-picker";
import AIEventCreator from "./_components/ai-cevent-creator";
...
export default function CreateEventPage() {
  ...
  // Use database-based Pro status instead of Clerk
  const hasPro = currentUser?.hasPro || false;
  const { mutate: createEvent, isLoading } = useConvexMutation(api.event.createEvent);
  const { mutate: upgradeToPro } = useConvexMutation(api.users.upgradeToPro); // Add upgradeToPro mutation
  ...
  const handleUpgradeToPro = async () => {
    try {
      await upgradeToPro({ hasPro: true });
      toast.success("Upgraded to Pro successfully! 🎉");
      // The page will re-render automatically with the new Pro status
    } catch (error) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await createEvent({
        ...
        // Remove hasPro from here - it will be determined internally in the mutation
      });
