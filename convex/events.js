...
export const getFeaturedEvents = query({
  ...
  handler: async (ctx, args) => {
    ...
    const events = await ctx.db.query("events").withIndex("by_startDate").filter((q)=>q.gte(q.field("startDate"),now)).order("desc").collect();
...
export const getEventsByLocation = query({
  ...
  handler: async (ctx, args) => {
    ...
    let events = await ctx.db.query("events").withIndex("by_startDate").filter((q)=>q.gte(q.field("startDate"),now)).collect();
...
export const getPopularEvents = query({
  ...
  handler: async (ctx, args) => {
    ...
    const events = await ctx.db.query("events").withIndex("by_startDate").filter((q)=>q.gte(q.field("startDate"),now)).collect();
